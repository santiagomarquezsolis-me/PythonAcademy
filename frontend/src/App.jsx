...
function App() {
  ...
  if (gamePhase === 'LANDING') {
    content = <LandingPage onStart={() => setGamePhase('CINEMATIC')} />;
  } else if (gamePhase === 'CINEMATIC') {
    content = <CinematicIntro onNext={() => setGamePhase('START')} />;
  } else if (gamePhase === 'START') {
    content = <StartScreen onStart={() => setGamePhase('HERO')} />;
  } else if (gamePhase === 'HERO') {
    content = <HeroScreen onNext={() => setGamePhase('NONO')} />;
  } else if (gamePhase === 'NONO') {
    content = <NonoScreen onContinue={() => setGamePhase('PLAYING')} />;
  } else {
    content = (
      <div style={{margin: 0, padding: 0, height: '100%', width: '100%', backgroundColor: '#0f0f13', overflow: 'hidden', boxSizing: 'border-box'}}>
        <MapEngine 
        ...
        {result && !nonoSuccessLevel && (
          <div style={{
            position: 'fixed', bottom: 50, right: 20, 
